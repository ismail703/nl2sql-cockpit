...
class SupervisorAgent:
    ...
    def generate_task_node(self, state: SupervisorState):        
        ...
        return {"task_description": response.content}
    ...
    def dispatch_sub_queries(self, state: SupervisorState):
            ...
            if len(sub_questions) > 0:
                print(f"\nDispatching {len(sub_questions)} sub-queries in parallel...")
                send_actions = []
    ...
    def store_memory_node(self, state: SupervisorState, config: RunnableConfig):
        ...
        try:
            ...
            if not candidate_lesson or candidate_lesson.upper() == "NONE":
                print("No lesson worth storing for this task.")
                return {}
            ...
            if decision.action == "add":
                ...
                print(f"Lesson added: {entry_id} -> {candidate_lesson}")
                return {
                ...
                print(f"Lesson updated: {decision.target_id} -> {decision.final_lesson}")
                return {
                    "messages": [AIMessage(content="Thank you for your feedback! I will consider it for future responses.")]
                } 
                ...
                print(f"Lesson deleted: {decision.target_id}")
                return {
                    "messages": [AIMessage(content="Thank you for your feedback! I will consider it for future responses.")]
                }

            else:
                print("Skipped — duplicate of existing lesson.")

        except Exception as e:
            print(f"Failed to extract/reconcile/store lesson: {e}")
    ...
    def create_workflow(self, checkpointer):
        ...
        workflow.add_conditional_edges(
            "reasoning",
            tools_condition,
            {"tools": "tools", "__end__": 'store_memory'}
        )
